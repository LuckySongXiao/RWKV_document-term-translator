...
namespace DocumentTranslator.Services.RwkvLocal
{
    ...
    public class ConcurrencyCalculator
    {
        ...
        /// <param name="apiUrl">rwkv_lightning API 地址</param>
        /// <param name="enableBszTest">是否启用并发上限测试</param>
        /// <param name="bszIncrement">并发测试每次递增的数量</param>
        /// <returns>计算得出的最大并发数</returns>
        public async Task<int> CalculateMaxConcurrencyAsync(GpuInfo gpu, ModelInfo model, string? apiUrl = null, bool enableBszTest = false, int bszIncrement = 10)
        {
            try
            {
                // 刷新GPU状态获取最新显存信息
                ...
                {
                    // 检查是否有缓存的并发数数据
                    var cachedConcurrency = _benchmarkCache.GetMaxBatchSize(apiUrl, gpu.Index);
                    ...
                    if (enableBszTest)
                    {
                        _logger.LogInformation($"开始 HTTP 并发上限测试，增量值: {bszIncrement}");
                        var benchmarkResult = await TestAllScenariosAsync(apiUrl, gpu, bszIncrement);
